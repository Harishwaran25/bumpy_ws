...
def main(args=None):
    ...
    bumpy1_controller = RobotController('bumpy1')
    bumpy2_controller = RobotController('bumpy7')
    bumpy3_controller = RobotController('bumpy6')
    
    # Create a MultiThreadedExecutor to run both nodes concurrently
    from rclpy.executors import MultiThreadedExecutor
    executor = MultiThreadedExecutor()
    executor.add_node(bumpy1_controller)
    executor.add_node(bumpy2_controller)
    executor.add_node(bumpy3_controller)
    ...
    try:
        ...
        # Clean shutdown
        executor.shutdown()
        bumpy1_controller.destroy_node()
        bumpy2_controller.destroy_node()
        bumpy3_controller.destroy_node()
        rclpy.shutdown()
